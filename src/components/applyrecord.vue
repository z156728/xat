	...
	export default {
		...
		data() {
			return {
				...
				isactive: 0,
				applyRecordLists: [],
				token: this.$route.params.appylRecords.split(',')[0],
				userId: this.$route.params.appylRecords.split(',')[1],
				classId: this.$route.params.appylRecords.split(',')[2],
				os: this.$route.params.appylRecords.split(',')[3]
			}
		},
		methods: {
			apply: function(index) {
				...
				this.$axios({
					method: 'post',
					url: '/student/getStudentApplyList',
					headers: {
						UserID: this.userId,
						Tonken: this.token,
						DeviceID: '',
